...
object SimpInt {
    ...
    def evalExp(dlt:Delta, e:Exp):Either[ErrMsg, Const] = e match {
      ...
      // Lab 2 Task 1.1
      case VarExp(x) => dlt.get(x) match {
        case Some(v) => Right(v)
        case None => Left(s"undefined variable ${x}.")
      }
    ...
    given evalOne:Evaluable[Stmt] = new Evaluable[Stmt] {
      def eval(dlt:Delta, s:Stmt):Either[ErrMsg, Delta] = s match {
        ...
        case While(cond,body) => {
          def loop(dlt1:Delta):Either[ErrMsg, Delta] = for {
            c <- evalExp(dlt1, cond)
            dlt2 <- c match {
              case IntConst(_) => Left("int expression found in the while condition position.")
              case BoolConst(b) if b => for {
                dlt3 <- evalMany.eval(dlt1, body)
                dlt4 <- loop(dlt3)
              } yield dlt4
              case BoolConst(b) if !b => Right(dlt1)
            }
          } yield dlt2
          loop(dlt)
        }
